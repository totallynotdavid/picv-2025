"""
This script processes tsunami wave height data from multiple monitoring stations
and creates visualization plots using GMT (Generic Mapping Tools).
"""
...
def process_tsunami_data():
    """
    Main function to process tsunami data and generate visualizations.
    """
    # Define constants
    ...
    for _i, name in enumerate(station_names):
        ...
        first_nonzero[name] = nonzero_indices[0] + 1 if len(nonzero_indices) > 0 else 0

        # Find maximum value
        max_values[name] = np.max(station_data[name])

    # Write to ttt_max.dat
    print("Writing ttt_max.dat...")
    try:
        ...
        print(f"Error writing ttt_max.dat: {e}")
        return
    ...
    print("Se creó el archivo ttt_max.dat")
...
def plot_mareograma(scale, tick_type):
    ...
    cmd = (
        f"gmt psbasemap -JX{size} -R{region} -B{axis} -P -K -X0 -Y-{dy} -O >> {psfile}"
    )
    subprocess.run(cmd, shell=True)
    ...
    cmd = f"gmt pstext -JX{size} -R0/10/0/4 -K -O -V < {callao_text.name} >> {psfile}"
    subprocess.run(cmd, shell=True)
    ...
    subprocess.run(f"ps2eps {psfile} -f", shell=True)
    subprocess.run(f"rm {psfile}", shell=True)
