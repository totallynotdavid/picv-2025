...
def _sql(template: str) -> str:
    """Insert the fixed select list into a query template."""
...
def _notify(conn: psycopg.Connection[JobRow], external_id: uuid.UUID) -> None:
    """Notify listeners after the transaction commits the job update."""
...
def get_artifacts(app_job_id: str) -> list[ArtifactRef]:
    """Return the manifest recorded with a completed job."""
    external_id = as_uuid(app_job_id)
    with pooled() as conn:
        row = conn.execute(
